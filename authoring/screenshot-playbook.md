...
```bash
# ターミナル1: アプリ本体（撮影対象）
cd /Users/newt/projects/next-score-watcher && pnpm install && pnpm run dev   # :3000

# ターミナル2: ドキュメント（生成結果のプレビュー）
cd /Users/newt/projects/docs.score-watcher.com && pnpm install && pnpm run dev   # :4321
```

> アプリ本体は dev だと右下に Next.js Dev Tools のフローティングボタンが出ます。撮影前に §3 のスニペットで非表示にします。綺麗さ最優先なら `pnpm build && pnpm start` で撮るのも可（起動は遅い）。

---

## 1. 形式名 → 内部名 対応表

指示された形式名を、アプリ内部のルールキー（`/rules` の「作る」対象・mdx ファイル名・アセット名に使う）へ変換します。正準は `next-score-watcher/src/utils/rules.ts`。
...
IndexedDB はブラウザに永続するため、一度投入すれば同一プロファイルでは再投入不要です。
...
2. Dev Tools ボタン非表示 + アニメーション無効化（`browser_evaluate`）:
   ```js
   () => {
...
`src/content/docs/rules/{内部名}.mdx` を以下テンプレートで作成/更新します。本文は `next-score-watcher/docs/rules/{内部名}.md`（詳細仕様）と `src/utils/rules.ts`（表示名・初期値）から要約転記します。共通操作は `next-score-watcher/docs/common-operations.md` を元に、重複を避けて要点のみ＋リンクで案内。
...
```bash
cd /Users/newt/projects/docs.score-watcher.com
